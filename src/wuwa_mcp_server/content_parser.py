...
class ContentParser:
    ...
    def parse_main_content(self, content_data):
        ...
        if "modules" in content_data and content_data["modules"]:
            result["modules"] = self._parse_modules(
                content_data["modules"], target_modules
            )
    ...
    def _parse_component(self, component):
        ...
        if "tabs" in component and component["tabs"]:
            ...
            component_data["parsed_content"] = self._parse_html_content(
                component["content"]
            )

        return (
            {"title": component_title, "data": component_data}
            if component_data
            else None
        )
                ...
                if target_modules and module_title == ModuleType.CHARACTER_DATA.value:
                    if module["components"]:
                        ...
                        component_data = {
                            ...
                            "info_texts": [
                                info.get("text", "")
                                for info in role_data.get("info", [])
                                if info.get("text")
                            ],
                        }
    ...
    def _convert_tag_to_markdown(self, tag):
        ...
        if tag.name == "p":
            content = "".join(
                self._convert_tag_to_markdown(child) for child in tag.children
            ).strip()
            if content:
        ...
        #         markdown_parts.append(f"![{alt}]({src})")
            ...
            for item in tag.find_all("li", recursive=False):
                item_content = "".join(
                    self._convert_tag_to_markdown(child) for child in item.children
                ).strip()
                if item_content:
    ...
    def _convert_table_to_markdown(self, table_tag):
        ...
        header_texts = [
            " ".join(cell.get_text(strip=True).split()) for cell in header_cells
        ]
        ...
        for row in rows[1:]:
            ...
            if len(data_cells) == len(header_cells):
                row_texts = [
                    " ".join(cell.get_text(strip=True).split()) for cell in data_cells
                ]
    ...
    def _parse_html_content(self, html_content):
        ...
        try:
            ...
            markdown_output = "".join(
                self._convert_tag_to_markdown(child) for child in soup.children
            )
            ...
            return {
                "markdown_content": f"<error>Failed to parse HTML: {str(e)}</error>",
                "tables": [],
            }
